...
import { updateXeMayAsync } from './redux/xeMaySlice';
...
export default function EditScreen() {
  ...
  const { xeMays } = useSelector((state: RootState) => state.xeMay);
  
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [errors, setErrors] = useState<FormErrors>({});
  
  // Find the motorcycle by id - so sánh ID dưới dạng string
  const xeMay = xeMays.find(item => String(item.id) === id);
  ...
  useEffect(() => {
    ...
    console.log('Available motorcycles:', xeMays);
    console.log('Found motorcycle:', xeMay);
  }, [id, xeMays]);
  
  // Load form data when xeMay is available
  useEffect(() => {
    if (xeMay) {
      setFormData({
        ten_xe_ph53550: xeMay.ten_xe_ph53550,
        mau_sac_ph53550: xeMay.mau_sac_ph53550,
        gia_ban_ph53550: xeMay.gia_ban_ph53550.toString(),
        mo_ta_ph53550: xeMay.mo_ta_ph53550,
        hinh_anh_ph53550: xeMay.hinh_anh_ph53550,
      });
    } else {
      // If no motorcycle found, go back
      Alert.alert('Lỗi', 'Không tìm thấy xe máy!');
      router.back();
    }
  }, [xeMay]);
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      try {
        ...
        // Truyền ID dưới dạng string
        await dispatch(updateXeMayAsync({ id, xeMay: submissionData }) as any);
        Alert.alert('Thành công', 'Cập nhật xe máy thành công!');
        router.back();
      } catch (error) {
        console.error('Error in handleSubmit:', error);
        Alert.alert('Lỗi', 'Có lỗi xảy ra khi cập nhật xe máy.');
      } finally {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      <Animatable.View animation="fadeInDown" duration={1000} style={styles.formContainer}>
        <Text style={styles.title}>Sửa Thông Tin Xe Máy</Text>
        ...
        <CustomInput
          label="Giá bán (VNĐ)"
          value={formData.gia_ban_ph53550}
        ...
        <View style={styles.buttonContainer}>
          <CustomButton 
            title={isSubmitting ? "Đang xử lý..." : "Cập Nhật"} 
